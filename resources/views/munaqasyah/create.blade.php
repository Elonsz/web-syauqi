...
@section('title', 'Tambah Santri & Nilai Munaqasyah')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-extrabold text-slate-800">Tambah Data Santri & Penilaian</h2>
            <p class="text-xs text-slate-500">Input biodata peserta munaqasyah, upload foto kelulusan, dan input 9 mata uji.</p>
        </div>
        <a href="{{ route('munaqasyah.index', ['jenis' => $jenis]) }}" class="text-xs text-slate-600 hover:text-slate-900 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-xs">
    ...
    <form action="{{ route('munaqasyah.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1 flex flex-col items-center justify-center p-4 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl">
                    ...
                    <label class="cursor-pointer bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg shadow-xs transition">
                        <i class="fa-solid fa-camera mr-1"></i> Upload Foto Siswa
                        <input type="file" name="foto" id="foto-input" accept="image/*" class="hidden" onchange="previewImage(this)">
                    </label>
                    <p class="text-[10px] text-slate-400 mt-1.5 text-center">Format: JPG, PNG, WEBP (Maks. 2MB). Foto akan tampil di ijazah/surat kelulusan.</p>
                </div>
                ...
                <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="jenis" class="w-full text-xs p-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                            <option value="TPQ" {{ $jenis == 'TPQ' ? 'selected' : '' }}>TPQ (Taman Pendidikan Al-Qur'an)</option>
                            <option value="RTQ" {{ $jenis == 'RTQ' ? 'selected' : '' }}>RTQ (Rumah Tahfidz Al-Qur'an)</option>
                        </select>
                    </div>
        ...
        <!-- CARD 2: NILAI MUNAQASYAH (BACAAN, HAFALAN & TERTULIS) -->
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
            <div class="border-b border-slate-100 pb-3 flex items-center justify-between">
                <h3 class="text-sm font-bold text-slate-800 flex items-center gap-2">
                    <i class="fa-solid fa-list-check text-amber-500"></i>
                    2. Nilai Ujian Munaqasyah (9 Komponen Penilaian)
                </h3>
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <span class="text-[10px] uppercase font-bold text-amber-800 tracking-wider">Perhitungan Otomatis Sistem:</span>
